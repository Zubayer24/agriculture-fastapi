...
from app.schemas.farm_schema import FarmSummaryResponse,FarmPerformanceResponse,TopFarmResponse, LossAnalysisResponse
from app.services.farm_service import get_farm_summary,get_farm_performance,get_top_farms, get_loss_analysis
from app.utils.validators import (VALID_YEARS,validate_filter, VALID_REGIONS,VALID_FARM_TYPES, VALID_CROP_CATEGORIES,VALID_MARKET_TYPES,VALID_GROWING_SEASONS,VALID_SEASONS,VALID_YEARS,VALID_QUALITY_GRADES ) 
...
@router.get("/farms/summary", response_model=FarmSummaryResponse) 
...
def farm_summary(
    ...
    farm_type: str = Query(None, description="Filter by farm type"),
    ...
    try:
        ...
        farm_type = validate_filter(farm_type,VALID_FARM_TYPES,"farm_type")
    ...
    grouped_df = get_farm_summary(region,farm_type,year,season)

        # Round avg_loss_pct to 1 decimal place
    grouped_df["avg_loss_pct"] = (grouped_df["avg_loss_pct"].round(1)) 
    ...
    return {"total_farms": grouped_df.shape[0],
...
@router.get("/farms/{farm_id}/performance",response_model=FarmPerformanceResponse,summary= "Single Farm Performance") 
...
def farm_performance(
    ...
    farm_df, performance_df = get_farm_performance(farm_id,year,crop_category,market_type) 
    ...
    farm_info = farm_df.to_dict(orient="records")[0]
...
@router.get("/farms/top",response_model=TopFarmResponse) 


def top_farms(

    metric: str = Query("profit",description="profit or revenue or yield"),
    ...
    limit: int = Query(10,description="Top N farms")):
    ...
    df = get_top_farms(metric,region,farm_type,year,limit)
...
@router.get("/farms/loss-analysis",response_model=LossAnalysisResponse) 
...
def loss_analysis(
    ...
    quality_grade: str = Query(None,description="Filter by quality grade"),
    ...
    try:
        ...
        growing_season = validate_filter(growing_season,VALID_GROWING_SEASONS,"growing_season") 

        quality_grade = validate_filter(quality_grade, VALID_QUALITY_GRADES,"quality_grade")

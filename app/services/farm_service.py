from app.utils.data_loaders import load_harvest_data, load_farm_profitability, load_farm_by_id
...
def get_farm_summary(
    ...
    grouped_df = (
        df.groupby(
            ["farm_name", "region", "farm_type"],as_index=False).agg(
...
def get_farm_performance(
    ...
    performance_df = (df.groupby(["crop_name", "year", "market_type","quality_grade"],as_index=False).agg(
...
def get_top_farms(
    ...
    df = df.sort_values(by="ranking_value",ascending=False)
...
def get_loss_analysis(
    ...
    # breakdown 
    breakdown_df = (df.groupby(
            ["region", "crop_category", "quality_grade", "pesticide_residue"],as_index=False).agg(
            total_lost_ton=("quantity_lost_ton", "sum"),
             total_harvested_ton=("quantity_harvested_ton", "sum")
        )
    )

    breakdown_df["loss_pct"] = (breakdown_df["total_lost_ton"]/ breakdown_df["total_harvested_ton"]) * 100 

    breakdown_df["loss_pct"] = (breakdown_df["loss_pct"].round(1))
